// Holds a persistent native-messaging port to the Claudication host, which
// pushes Claude Code's status whenever it changes. An open port also keeps
// this service worker alive.
const HOST = 'com.tpojka.claudication';
const RECONNECT_ALARM = 'reconnect';

const ICONS = {
  busy: 'knee-pain',
  ready: 'knee-ok',
  unknown: 'knee-unknown',
};
...
function render(state, detail) {
  const titles = {
    busy: 'Claude Code is working…',
    ready: 'Claude is ready',
    unknown: 'Claudication: native host not connected',
  };
