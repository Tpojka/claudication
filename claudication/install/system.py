...
def write_host_launcher(app):
    """Chrome starts native hosts as executables, so wrap `python claudication.pyz host` in a script."""
    if sys.platform == "win32":
        launcher = paths.data_dir() / "claudication-host.bat"
        with open(launcher, "w", newline="\r\n") as f:
            f.write(f'@echo off\n"{sys.executable}" "{app}" host %*\n')
    else:
        launcher = paths.data_dir() / "claudication-host"
