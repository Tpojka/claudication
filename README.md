...
This branch contains only the macOS code. The `all` branch supports every OS and detects which one you're on.
...
- macOS notifications through the built-in `osascript`, with the Glass sound. The first time, allow notifications for **Script Editor** in System Settings → Notifications.
